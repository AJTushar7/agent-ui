    ...
    <div class="admin-container">
      <div class="header">
        ...
        <div class="button-group">
          <button class="create-user-btn" mat-raised-button color="primary" (click)="addNewUser()" [disabled]="loading">
            Create User
          </button>
        </div>
      </div>
...
export class AdminComponent implements OnInit {
    ...
    loadUsers(page: number = 1) {
      ...
      this.http.get<{ users: User[], total: number }>(`http://localhost:8000/auth/login-user-list?page=${page}&per_page=${this.perPage}`,{headers})
        .subscribe({
          next: (res) => {
